...
contract RC4 {
    ...
    function  rc4_encrypt_decrypt_bytes32(bytes32  input, bytes32  key) public pure returns (bytes32 output) {
        bytes memory input_bytes = abi.encodePacked(input);
        bytes memory key_bytes = abi.encodePacked(key);

        bytes memory output_bytes = rc4_encrypt_decrypt_bytes(input_bytes, key_bytes);
    ...
    function  rc4_encrypt_decrypt_bytes(bytes memory input, bytes memory key) public pure returns (bytes memory output) {
        //Generate Key Schedule State
        bytes memory state = abi.encodePacked(  uint(0x000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f),
